...
class CSFAlgorithmAnimation(Scene):
    ...
    def physics_simulation(self):
        """Animate improved physics simulation"""
        ...
        # Animate cloth falling and settling with better physics
        all_points = np.vstack([self.ground_points, self.vegetation_points, self.building_points])
        
        # Add velocity tracking for more realistic physics
        cloth_velocities = np.zeros_like(self.cloth_points)
        ...
        for iteration in range(iterations):
            ...
            for i, (x, y, z) in enumerate(self.cloth_points):
                current_velocity = cloth_velocities[i]
                ...
                new_x, new_y, new_z = [x, y, z] + current_velocity
                
                # Enhanced collision detection with multiple nearby points
                nearby_points = all_points[
                    (np.abs(all_points[:, 0] - new_x) < 0.4) & 
                    (np.abs(all_points[:, 1] - new_y) < 0.4)
                ]
                ...
                if len(nearby_points) > 0:
                    ...
                    if new_z < max_nearby_z + collision_buffer:
                        ...
                        current_velocity[2] = max(0, current_velocity[2] * 0.3)  # Bounce with energy loss
                
                # Add some spring forces between cloth particles (simplified)
                if iteration > 5:  # Start spring forces after initial fall
                    spring_force = np.array([0.0, 0.0, 0.0])
                    neighbors_found = 0
                    
                    for j, (ox, oy, oz) in enumerate(self.cloth_points):
                        if i != j:
                            distance = np.sqrt((new_x - ox)**2 + (new_z - oz)**2)
                            if distance < 0.8:  # Within spring range
                                if distance > 0.1:  # Avoid division by zero
                                    spring_direction = np.array([ox - new_x, 0, oz - new_z]) / distance
                                    spring_force += spring_direction * 0.02
                                    neighbors_found += 1
                    
                    if neighbors_found > 0:
                        spring_force /= neighbors_found
                        current_velocity += spring_force
                
                new_cloth_positions.append([new_x, new_y, new_z])
                new_velocities.append(current_velocity)
            
            self.cloth_points = np.array(new_cloth_positions)
            cloth_velocities = np.array(new_velocities)
            
            # Update visualization every few iterations with smoother animation
            if iteration % 2 == 0:  # Update every other iteration
                new_cloth_dots = VGroup(*[
                    Dot(self.axes.coords_to_point(x, z), color=BLUE, radius=0.02) 
                    for x, y, z in self.cloth_points
                ])
                
                # Update cloth lines too
                new_cloth_lines = VGroup()
                x_coords = np.unique(self.cloth_points[:, 0])
                y_coords = np.unique(self.cloth_points[:, 2])
                
                # Redraw grid lines based on new positions
                for i, cloth_point in enumerate(self.cloth_points):
                    x, y, z = cloth_point
                    # Connect to nearby points to maintain grid appearance
                    nearby_cloth = self.cloth_points[
                        (np.abs(self.cloth_points[:, 0] - x) < 0.6) & 
                        (np.abs(self.cloth_points[:, 2] - z) < 0.6)
                    ]
                    
                    for nearby_point in nearby_cloth[:2]:  # Limit connections
                        if not np.array_equal(nearby_point, cloth_point):
                            line = Line(
                                self.axes.coords_to_point(x, z),
                                self.axes.coords_to_point(nearby_point[0], nearby_point[2]),
                                color=BLUE, stroke_width=0.5
                            )
        ...
        settled_explanation = Text("Cloth settles on surface features with realistic physics", 
