...
class CSFAlgorithmAnimation(Scene):
    def construct(self):
        ...
        self.physics_simulation_true_csf_algorithm()
    ...
    def physics_simulation_true_csf_algorithm(self):
        """True CSF Algorithm: Invert cloud, drop cloth, classify, then flip back"""
        title = Text("True CSF Algorithm (Zhang et al. 2016)", font_size=48, color=BLUE)
        title.to_edge(UP)
        
        self.play(Write(title))
        
        # STEP 1: Show original point cloud
        all_points = np.vstack([self.ground_points, self.vegetation_points, self.building_points])
        
        step1_title = Text("Step 1: Original Point Cloud", font_size=24, color=GREEN)
        step1_title.to_edge(DOWN)
        self.play(Write(step1_title))
        
        # Show original points with type colors (for educational clarity)
        ground_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, z), color=BROWN, radius=0.025) 
            for x, y, z in self.ground_points
        ])
        vegetation_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, z), color=GREEN, radius=0.025) 
            for x, y, z in self.vegetation_points
        ])
        building_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, z), color=GRAY, radius=0.025) 
            for x, y, z in self.building_points
        ])
        ...
        # STEP 2: Remove original cloud, then show inverted cloud
        step2_title = Text("Step 2: Invert Point Cloud (flip upside down)", font_size=24, color=GREEN)
        step2_title.to_edge(DOWN)
        self.play(Transform(step1_title, step2_title))
        
        # Calculate inversion - flip around horizontal axis
        max_z = all_points[:, 2].max()
        min_z = all_points[:, 2].min()
        z_center = (max_z + min_z) / 2
        
        # Create inverted points
        inverted_ground_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, 2*z_center - z), color=BROWN, radius=0.025) 
            for x, y, z in self.ground_points
        ])
        inverted_vegetation_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, 2*z_center - z), color=GREEN, radius=0.025) 
            for x, y, z in self.vegetation_points
        ])
        inverted_building_dots = VGroup(*[
            Dot(self.axes.coords_to_point(x, 2*z_center - z), color=GRAY, radius=0.025) 
            for x, y, z in self.building_points
        ])
        
        # FIRST: Remove original cloud completely - force removal
        self.remove(ground_dots, vegetation_dots, building_dots)
        
        # Also clear from mobjects list if still there
        for obj in [ground_dots, vegetation_dots, building_dots]:
            if obj in self.mobjects:
                self.mobjects.remove(obj)
        
        self.wait(1)
        
        # THEN: Show inverted cloud
        self.play(
            FadeIn(inverted_ground_dots),
            FadeIn(inverted_vegetation_dots), 
            FadeIn(inverted_building_dots),
            run_time=1.5
        )
        self.wait(2)
        
        # STEP 3: Initialize cloth above inverted surface
        step3_title = Text("Step 3: Initialize cloth above inverted surface", font_size=24, color=GREEN)
        step3_title.to_edge(DOWN)
        self.play(Transform(step1_title, step3_title))
        
        # Create inverted point coordinates for physics
        inverted_points = all_points.copy()
        inverted_points[:, 2] = 2*z_center - inverted_points[:, 2]
        
        # Separate inverted points by type for proper collision detection
        inverted_ground_points = self.ground_points.copy()
        inverted_ground_points[:, 2] = 2*z_center - inverted_ground_points[:, 2]
        
        inverted_vegetation_points = self.vegetation_points.copy()
        inverted_vegetation_points[:, 2] = 2*z_center - inverted_vegetation_points[:, 2]
        
        inverted_building_points = self.building_points.copy()
        inverted_building_points[:, 2] = 2*z_center - inverted_building_points[:, 2]
        
        # Find bounds of inverted cloud
        min_x, max_x = inverted_points[:, 0].min(), inverted_points[:, 0].max()
        min_y, max_y = inverted_points[:, 1].min(), inverted_points[:, 1].max()
        inverted_max_z = inverted_points[:, 2].max()
        
        # Cloth parameters
        resolution = 0.3
        padding = resolution
        x_coords = np.arange(min_x - padding, max_x + padding, resolution)
        y_coords = np.arange(min_y - padding, max_y + padding, resolution)
        
        # Initialize cloth ABOVE inverted surface
        cloth_start_height = inverted_max_z + 1.5
        cloth_heights = {}
        cloth_velocities = {}
        cloth_stuck = {}  # Track which particles are stuck
        
        for i, x in enumerate(x_coords):
            for j, y in enumerate(y_coords):
                cloth_heights[(i, j)] = cloth_start_height
                cloth_velocities[(i, j)] = 0.0
                cloth_stuck[(i, j)] = False
        
        print(f"Cloth initialized at {cloth_start_height:.2f}m above inverted surface (max_z: {inverted_max_z:.2f})")
        
        # Visualize initial cloth
        def create_cloth_visual(heights_dict, stuck_dict=None, color=BLUE):
            cloth_dots = VGroup()
            
            for (i, j), height in heights_dict.items():
                x = x_coords[i] if i < len(x_coords) else 0
                
                # Color based on stuck state
                if stuck_dict and stuck_dict.get((i, j), False):
                    dot_color = RED  # Stuck particles in red
                    radius = 0.025
                else:
                    dot_color = color  # Moving particles in blue
                    radius = 0.02
                
                dot = Dot(self.axes.coords_to_point(x, height), color=dot_color, radius=radius)
                cloth_dots.add(dot)
            ...
            for i in range(len(x_coords) - 1):
                for j in range(len(y_coords) - 1):
                    if (i, j) in heights_dict and (i+1, j) in heights_dict:
                        start = self.axes.coords_to_point(x_coords[i], heights_dict[(i, j)])
                        end = self.axes.coords_to_point(x_coords[i+1], heights_dict[(i+1, j)])
                        line = Line(start, end, color=color, stroke_width=1, stroke_opacity=0.7)
                        cloth_lines.add(line)
                    
                    if (i, j) in heights_dict and (i, j+1) in heights_dict:
                        start = self.axes.coords_to_point(x_coords[i], heights_dict[(i, j)])
                        end = self.axes.coords_to_point(x_coords[i], heights_dict[(i, j+1)])
                        line = Line(start, end, color=color, stroke_width=1, stroke_opacity=0.7)
                        cloth_lines.add(line)
            
            return cloth_dots, cloth_lines
        
        cloth_dots, cloth_lines = create_cloth_visual(cloth_heights, cloth_stuck)
        self.play(FadeIn(cloth_dots), Create(cloth_lines))
        self.wait(2)
        
        # STEP 4: Cloth simulation with immediate sticking to ground points
        step4_title = Text("Step 4: Cloth falls and STICKS to ground points immediately", font_size=24, color=GREEN)
        step4_title.to_edge(DOWN)
        self.play(Transform(step1_title, step4_title))
        
        # Enhanced CSF physics with immediate ground sticking
        gravity = 0.15
        damping = 0.85
        time_step = 0.5
        spring_strength = 0.8  # Strong springs for cloth integrity
        iterations = 25
        ...
        for iteration in range(iterations):
            ...
            for (i, j), height in cloth_heights.items():
                x = x_coords[i] if i < len(x_coords) else 0
                y = y_coords[j] if j < len(y_coords) else 0
                velocity = cloth_velocities[(i, j)]
                is_stuck = cloth_stuck[(i, j)]
                
                if is_stuck:
                    # Stuck particles don't move
                    new_heights[(i, j)] = height
                    new_velocities[(i, j)] = 0.0
                    new_stuck[(i, j)] = True
                    continue
                
                # Apply gravity to free particles
                velocity -= gravity * time_step
                
                # Spring forces from neighbors (maintain cloth structure)
                spring_force = 0.0
                neighbor_count = 0
                ...
                for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    ...
                    if (ni, nj) in cloth_heights:
                        neighbor_height = cloth_heights[(ni, nj)]
                        neighbor_stuck = cloth_stuck[(ni, nj)]
                        
                        height_diff = neighbor_height - height
                        
                        # Stronger springs if neighbor is stuck (anchor points)
                        spring_factor = spring_strength * 2 if neighbor_stuck else spring_strength
                        
                        # Limit displacement to maintain cloth integrity
                        max_diff = 0.4
                        if abs(height_diff) > max_diff:
                            height_diff = np.sign(height_diff) * max_diff
                        
                        spring_force += spring_factor * height_diff
                        neighbor_count += 1
                
                if neighbor_count > 0:
                    velocity += (spring_force / neighbor_count) * time_step
                ...
                # CRITICAL: Collision detection with IMMEDIATE STICKING to ground points
                search_radius = resolution * 1.2
                
                # Check collision with GROUND POINTS specifically (these should stick)
                nearby_ground = inverted_ground_points[
                    (np.abs(inverted_ground_points[:, 0] - x) < search_radius) &
                    (np.abs(inverted_ground_points[:, 1] - y) < search_radius)
                ]
                
                stuck_to_ground = False
                if len(nearby_ground) > 0:
                    ground_surface = nearby_ground[:, 2].max()
                    stick_distance = collision_buffer * 2  # Sticky zone around ground
                    
                    if new_height <= ground_surface + stick_distance:
                        # IMMEDIATE STICKING to ground
                        new_height = ground_surface + collision_buffer
                        velocity = 0.0  # Complete stop
                        stuck_to_ground = True
                        print(f"Cloth particle ({i},{j}) STUCK to ground at height {new_height:.2f}")
                
                # Check collision with buildings (less sticky, cloth can slide over)
                if not stuck_to_ground:
                    nearby_buildings = inverted_building_points[
                        (np.abs(inverted_building_points[:, 0] - x) < search_radius) &
                        (np.abs(inverted_building_points[:, 1] - y) < search_radius)
                    ]
                    
                    if len(nearby_buildings) > 0:
                        building_surface = nearby_buildings[:, 2].max()
                        
                        if new_height <= building_surface + collision_buffer:
                            new_height = building_surface + collision_buffer
                            velocity *= 0.4  # Partial damping, can still slide
                
                # Vegetation offers minimal resistance (cloth passes through mostly)
                if not stuck_to_ground:
                    nearby_vegetation = inverted_vegetation_points[
                        (np.abs(inverted_vegetation_points[:, 0] - x) < search_radius) &
                        (np.abs(inverted_vegetation_points[:, 1] - y) < search_radius)
                    ]
                    
                    if len(nearby_vegetation) > 0:
                        # Vegetation only provides slight resistance, no sticking
                        velocity *= 0.95  # Minimal damping
                
                new_heights[(i, j)] = new_height
                new_velocities[(i, j)] = velocity
                new_stuck[(i, j)] = stuck_to_ground
            
            # Update cloth state
            cloth_heights = new_heights
            cloth_velocities = new_velocities
            cloth_stuck = new_stuck
            
            # Update visualization every few iterations to show sticking behavior
            if iteration % 2 == 0:
                new_cloth_dots, new_cloth_lines = create_cloth_visual(cloth_heights, cloth_stuck)
                
                # Show progress
                stuck_count = sum(cloth_stuck.values())
                progress_text = Text(f"Iteration {iteration+1}/{iterations} - {stuck_count} particles stuck", 
                ...
                if iteration == 0:
                    ...
                    self.play(
                        Transform(cloth_dots, new_cloth_dots),
                        Transform(cloth_lines, new_cloth_lines),
                        Transform(getattr(self, 'progress_text', progress_text), progress_text),
                        run_time=0.4
                    )
        ...
        # STEP 5: Filter points - REMOVE those beyond distance threshold
        step5_title = Text("Step 5: Filter points - REMOVE those > 5mm from cloth", font_size=24, color=GREEN)
        step5_title.to_edge(DOWN)
        self.play(Transform(step1_title, step5_title))
        
        # Remove cloth first to see point filtering clearly
        animations = [FadeOut(cloth_dots, cloth_lines)]
        if hasattr(self, 'progress_text'):
            animations.append(FadeOut(self.progress_text))
        self.play(*animations)
        self.wait(1)
        
        # Filter points in inverted space - KEEP only those within threshold
        threshold = 0.005  # 5mm threshold as you specified
        points_to_keep = []  # Only points within threshold distance
        points_removed = []  # Points that get filtered out
        
        for point in inverted_points:
            x, y, z = point
            
            # Find nearest cloth height using proper grid lookup
            grid_i = int(round((x - x_coords[0]) / resolution))
            grid_j = int(round((y - y_coords[0]) / resolution)) if len(y_coords) > 1 else 0
            
            grid_i = max(0, min(len(x_coords) - 1, grid_i))
            grid_j = max(0, min(len(y_coords) - 1, grid_j))
            
            if (grid_i, grid_j) in cloth_heights:
                cloth_height = cloth_heights[(grid_i, grid_j)]
                distance_to_cloth = abs(z - cloth_height)
                
                # CSF filtering: KEEP only points within threshold
                if distance_to_cloth <= threshold:
        ...
        print(f"CSF Filtering: {len(points_to_keep)} points kept, {len(points_removed)} points removed (threshold: {threshold*1000:.0f}mm)")
        
        # Show filtering process - fade out points that are too far
        if points_removed:
            points_to_remove_dots = VGroup(*[
                Dot(self.axes.coords_to_point(x, z), color=RED, radius=0.02)
                for x, y, z in points_removed
            ])
            
            # Show points being removed
            removal_explanation = Text(f"Removing {len(points_removed)} points > {threshold*1000:.0f}mm from cloth", 
                                    font_size=20, color=RED)
            removal_explanation.to_edge(DOWN)
            self.play(Transform(step1_title, removal_explanation))
            
            # Highlight points to be removed
            self.play(FadeIn(points_to_remove_dots, lag_ratio=0.01))
            self.wait(1)
            
            # Remove them
            self.play(FadeOut(points_to_remove_dots, inverted_ground_dots, inverted_vegetation_dots, inverted_building_dots))
            self.wait(1)
        
        # Show only the kept points (these become ground points)
        if points_to_keep:
            kept_points_dots = VGroup(*[
                Dot(self.axes.coords_to_point(x, z), color=BROWN, radius=0.03)
                for x, y, z in points_to_keep
            ])
            
            keep_explanation = Text(f"Keeping {len(points_to_keep)} ground points within {threshold*1000:.0f}mm", 
                                font_size=20, color=BROWN)
            keep_explanation.to_edge(DOWN)
            self.play(Transform(step1_title, keep_explanation))
            
            self.play(FadeIn(kept_points_dots, lag_ratio=0.02))
            self.wait(2)
            
            # Store for flip-back
            ground_classified = points_to_keep
            non_ground_classified = points_removed
        else:
            kept_points_dots = VGroup()
            ground_classified = []
            non_ground_classified = points_removed
        ...
        # STEP 6: Flip everything back to original orientation
        step6_title = Text("Step 6: Return to original orientation with classifications", font_size=24, color=GREEN)
        step6_title.to_edge(DOWN)
        self.play(Transform(step1_title, step6_title))
        
        # Remove inverted classified points (kept_points_dots from previous step)
        if 'kept_points_dots' in locals() and len(kept_points_dots) > 0:
            self.play(FadeOut(kept_points_dots))
        self.wait(0.5)
        
        # Convert classifications back to original coordinates
        original_ground_classified = []
        original_non_ground_classified = []
        
        for point in ground_classified:
            x, y, z = point
            original_z = 2*z_center - z  # Un-invert
            original_ground_classified.append([x, y, original_z])
        
        for point in non_ground_classified:
            x, y, z = point
            original_z = 2*z_center - z  # Un-invert
            original_non_ground_classified.append([x, y, original_z])
        
        # Show final results in original orientation
        final_ground_dots = VGroup()
        final_non_ground_dots = VGroup()
        
        if original_ground_classified:
            final_ground_dots = VGroup(*[
                Dot(self.axes.coords_to_point(x, z), color=BROWN, radius=0.03)
                for x, y, z in original_ground_classified
            ])
            
        if original_non_ground_classified:
            final_non_ground_dots = VGroup(*[
                Dot(self.axes.coords_to_point(x, z), color=RED, radius=0.03)
                for x, y, z in original_non_ground_classified
            ])
        
        # Display final results
        animations = []
        if len(final_ground_dots) > 0:
            animations.append(FadeIn(final_ground_dots, lag_ratio=0.02))
        if len(final_non_ground_dots) > 0:
            animations.append(FadeIn(final_non_ground_dots, lag_ratio=0.02))
        ...
        # Show final results
        results_text = VGroup(
            Text("CSF Algorithm Complete!", font_size=20, color=YELLOW),
            Text(f"Ground points (brown): {len(original_ground_classified)}", font_size=16, color=BROWN),
            Text(f"Non-ground points (red): {len(original_non_ground_classified)}", font_size=16, color=RED),
            Text(f"Classification threshold: {threshold*1000:.0f}mm", font_size=16, color=WHITE)
        )
        results_text.arrange(DOWN, aligned_edge=LEFT)
        results_text.to_corner(UR, buff=0.5)
        
        final_explanation = Text("Final: Ground points identified without physics bias!", 
                            font_size=24, color=GREEN)
        final_explanation.to_edge(DOWN)
        
        self.play(
            Transform(step1_title, final_explanation),
            FadeIn(results_text)
        )
        
        self.wait(4)
        
        # Store results for next scene
        self.physics_ground_classified = original_ground_classified
        self.physics_non_ground_classified = original_non_ground_classified
        
        # Cleanup
        cleanup_objects = [title, step1_title, results_text]
        if len(final_ground_dots) > 0:
            cleanup_objects.append(final_ground_dots)
        if len(final_non_ground_dots) > 0:
            cleanup_objects.append(final_non_ground_dots)
